...
for p in Airplane:
    ...
    for i,j in Distance:
        for ID in yDividers[i,j,p].ids:
            for k in range(10):
                if name2solutionValue["y#" + i + "_" + j + "_" + p + "_" + ID] > 0.1+k: 
                    yString+=["y#" + i + "_" + j +  "_" +p + "_" + ID]
                    flyTime[p]+=Traveltime[i,j,p]
                else:
                    break
    ...
    for ID in yDepDividers[p].ids:
        if name2solutionValue["y_dep#" + p + "_" + ID] > 0.1:
            earliestDep=max([min(yDepDividers[p].ids[ID]),0])
            startIndex=(p,ID)
    latestArr=0

    for ID in yArrDividers[p].ids:
        if name2solutionValue["y_arr#" + p + "_" + ID] > 0.1:
            latestArr=min([max(yArrDividers[p].ids[ID]),number_of_timesteps])
            goalIndex=(p,ID)
    ...
    if len(paths[p])>1:
        for path in paths[p]:
            if path[0]!=Airplane[p].origin and path[0]==path[-1]:
                ...
                prevPath=0
                for i in range(1,len(path)):
                    prevPath+=Traveltime[path[i-1],path[i],p]
                    yDividers[path[i-1],path[i],p].addDivider([1],number_of_timesteps)
                    LOP=[prevPath+m*cycleLength for m in range(1+int(number_of_timesteps) % int(cycleLength)) if prevPath+m*cycleLength <= number_of_timesteps]
                    if LOP != []:
                        print("Cycle Divider added")
                        yDividers[path[i-1],path[i],p].addDivider(LOP,number_of_timesteps)
    ...
    if flyTime[p] > latestArr-earliestDep-0.0001:
        for path in paths[p]:
            ...
            for i in range(len(path)-1):
                """
                if path[i]==0:
                    yDepDividers[p].addDivider([1],number_of_timesteps)
                if i==len(path)-2:
                    yArrDividers[p].addDivider([1],number_of_timesteps)
                """
                yDividers[path[i],path[i+1],p].addDivider([1],number_of_timesteps)
                LOP=[prevPath+1+m*pathLength for m in range(int(number_of_timesteps) % int(pathLength)) if prevPath+m*pathLength <= number_of_timesteps]
                if LOP != []:
                    yDividers[path[i],path[i+1],p].addDivider(LOP,number_of_timesteps)
                    """
                    if i==0 and Airplane[p].origin==path[i]:
                        yDepDividers[p].addDivider(LOP,number_of_timesteps)
                    if i==len(path)-2 and Airplane[p].destination==path[i+1]:
                        yArrDividers[p].addDivider(LOP,number_of_timesteps)
                    """
                    print("Path Divider added")
    ...
    for i,j in Distance:
        for t in tS:
            #if name2solutionValue["y#" + i + "_" + j + "_" + p + "_" + yDividers[i,j,p].findId(t)] > 0.1:
    ...
    for t in tS:
        if name2solutionValue[ "y_dep#" + p  + "_" + yDepDividers[p].findId(t)] > 0.1:
            y_dep2[p,t] = "y_dep#" + p  + "_" + str(t)
            model2.variables.add(names = [y_dep2[p,t]], lb = [0.0], ub = [1.0], types = ["B"])
        else:
            y_dep2[p,t] = "y_dep#" + p  + "_" + str(t)
            model2.variables.add(names = [y_dep2[p,t]], lb = [0.0], ub = [1.0], types = ["B"])
    ...
    for t in tS:
        if name2solutionValue[ "y_arr#" + p  + "_" + yArrDividers[p].findId(t)] > 0.1:
            y_arr2[p,t] = "y_arr#" + p  + "_" + str(t)
            model2.variables.add(names = [y_arr2[p,t]], lb = [0.0], ub = [1.0], types = ["B"])
        else:
            y_arr2[p,t] = "y_arr#" + p  + "_" + str(t)
            model2.variables.add(names = [y_arr2[p,t]], lb = [0.0], ub = [1.0], types = ["B"])
    """
    ...
    for r in Request:
        xString[r]=[]
        for i,j in Distance:
            for t in xDividers[i,j,r,p].ids:
                if name2solutionValue[ "x#" + i + "_" + j + "_" + r + "_" + p + "_" + str(t)] > 0.1:
                    xString[r]+=["x#" + i + "_" + j + "_" + r + "_" + p + "_"+str(t)]
    
    for r in Request:
        for i,j in Distance:
            for t in tS:
                if name2solutionValue[ "x#" + i + "_" + j + "_" + r + "_" + p + "_" + xDividers[i,j,r,p].findId(t)] > 0.1:
                    x2[i,j,r,p,t] = "x#" + i + "_" + j + "_" + r + "_" + p + "_"+str(t)
                    model2.variables.add( names = [x2[i,j,r,p,t]], lb = [0.0], ub = [1.0], types = ["B"])
                    assignedRequests[r]=Request[r]
                else:
                    x2[i,j,r,p,t] = "x#" + i + "_" + j + "_" + r + "_" + p + "_"+str(t)
                    model2.variables.add( names = [x2[i,j,r,p,t]], lb = [0.0], ub = [1.0], types = ["B"])
    ...
    for r in Request:
        for t in tS:
            if name2solutionValue[  "x_dep#" + r + "_" + p + "_" + xDepDividers[r,p].findId(t)] > 0.1:
                x_dep2[r,p,t] = "x_dep#" + r + "_" + p + "_" + str(t)
                model2.variables.add(names = [x_dep2[r,p,t]], lb = [0], ub = [1], types = ["B"])
            else:
                x_dep2[r,p,t] = "x_dep#" + r + "_" + p + "_" + str(t)
                model2.variables.add(names = [x_dep2[r,p,t]], lb = [0], ub = [1.0], types = ["B"])
    
    x_arr2 = {}
    
    for r in Request:
        for t in tS:
            if name2solutionValue[  "x_arr#" + r + "_" + p + "_" + xArrDividers[r,p].findId(t)] > 0.1:
                x_arr2[r,p,t] = "x_arr#" + r + "_" + p + "_" + str(t)
                model2.variables.add(names = [x_arr2[r,p,t]], lb = [0], ub = [1], types = ["B"])
            else:
                x_arr2[r,p,t] = "x_arr#" + r + "_" + p + "_" + str(t)
                model2.variables.add(names = [x_arr2[r,p,t]], lb = [0], ub = [1.0], types = ["B"])
    ...
    for r in Request:
        ...
        model2.variables.add(obj = [0.1], names = [stopSlack[r]], lb = [0], types = ["I"])    
    ...
    for r in assignedRequests:
        ...
        #for t in xrange(Request[r].earliest_departure,Request[r].latest_arrival):
            ...
            thevars += [x2[ori,j,r,p,t+Traveltime[ori,j,p]]
                            for j in Airport if j != Request[r].origin and t+Traveltime[ori,j,p]< number_of_timesteps]
            thecoefs += [1.0 for j in Airport if j != Request[r].origin and t+Traveltime[ori,j,p]< number_of_timesteps]
        ...
        for i in Airport:
            for j in Airport:
                if (i!=j):
                    ...
                    for t in tS:
                        idToTimeSteps[xDividers[i,j,r,p].findId(t)].append(t)
                    
                    for ID in xDividers[i,j,r,p].ids:
                        thevars=[x2[i,j,r,p,t] for t in idToTimeSteps[ID]]
                        thecoefs=[1.0]*len(thevars)+[-1.0,1.0]
                        thevars+=[xSlack[i,j,r,p,ID],xSlack2[i,j,r,p,ID]]
                        rhs=solutionValues[name2idx[x[i,j,r,p,ID]]]
                        model2.linear_constraints.add(lin_expr = [cplex.SparsePair(thevars,thecoefs)], senses = ["E"], rhs = [rhs])
    ...
    for r in assignedRequests:    
        ...
        for t in tS:
            idToTimeSteps[xArrDividers[r,p].findId(t)].append(t)
        
        for ID in xArrDividers[r,p].ids:
            thevars=[x_arr2[r,p,t] for t in idToTimeSteps[ID]]
            thecoefs=[1.0]*len(thevars)+[-1.0,1.0]
            thevars+=[x_arrSlack[r,p,ID],x_arrSlack2[r,p,ID]]
            rhs=solutionValues[name2idx[x_arr[r,p,ID]]]
            model2.linear_constraints.add(lin_expr = [cplex.SparsePair(thevars,thecoefs)], senses = ["E"], rhs = [rhs])
    ...
    for r in assignedRequests:
        ...
        for t in tS:
            idToTimeSteps[xDepDividers[r,p].findId(t)].append(t)
        
        for ID in xDepDividers[r,p].ids:
            thevars=[x_dep2[r,p,t] for t in idToTimeSteps[ID]]
            thecoefs=[1.0]*len(thevars)+[-1.0,1.0]
            thevars+=[x_depSlack[r,p,ID],x_depSlack2[r,p,ID]]
            rhs=solutionValues[name2idx[x_dep[r,p,ID]]]
            model2.linear_constraints.add(lin_expr = [cplex.SparsePair(thevars,thecoefs)], senses = ["E"], rhs = [rhs])
    print("Slacks added, time passed: %f" % (time.clock() - time_start))         
    ...
    for r in assignedRequests:
        ...
        model2.linear_constraints.add(lin_expr = [cplex.SparsePair(thevars,thecoefs)], senses = ["L"], rhs = [max_stops + 1])
    ...
    if model2.solution.is_primal_feasible():
        if model2.solution.get_objective_value()>0.0001:
            for key,val in xSlack.iteritems():
                valStore=solutionValues2[name2idx2[val]]
                if valStore > 0.0001:
                    requestsToSplit[varToKey(val,3)]=1
            for key,val in seatSlack.iteritems():
                valStore=solutionValues2[name2idx2[val]]
                if valStore > 0.0001:
                    A1=varToKey(val,1)
                    A2=varToKey(val,2)
                    for key2,val2 in x.iteritems():
                        valStore=solutionValues[name2idx[val2]]
                        if valStore > 0.5 and varToKey(val2,1)==A1 and varToKey(val2,2)==A2:
                            requestsToSplit[varToKey(val2,3)]=1  
            for key,val in stopSlack.iteritems():
                valStore=solutionValues2[name2idx2[val]]
                if valStore > 0.0001:
                    requestsToSplit[varToKey(val,1)]=1
    
    
    
        for r in requestsToSplit:
            for path in xPaths[p,r]:
                pathLength=0
                for i in range(len(path)-1):
                    pathLength+=Traveltime[path[i],path[i+1],p]
                prevPath=0
                for i in range(len(path)-1):
                    if path[i]==0:
                        xDepDividers[r,p].addDivider([1],number_of_timesteps)
                    if i==len(path)-2:
                        xArrDividers[r,p].addDivider([1],number_of_timesteps)
                    xDividers[path[i],path[i+1],r,p].addDivider([1],number_of_timesteps)
                    yDividers[path[i],path[i+1],p].addDivider([1],number_of_timesteps)
                    LOP=[prevPath+1+m*pathLength for m in range(int(number_of_timesteps) % int(pathLength)) if prevPath+m*pathLength <= number_of_timesteps]
                    if LOP != []:
                        xDividers[path[i],path[i+1],r,p].addDivider(LOP,number_of_timesteps)
                        yDividers[path[i],path[i+1],p].addDivider(LOP,number_of_timesteps)
                        if i==0 and Request[r].origin==path[i]:
                            xDepDividers[r,p].addDivider(LOP,number_of_timesteps)
                        if i==len(path)-2 and Request[r].destination==path[i+1]:
                            xArrDividers[r,p].addDivider(LOP,number_of_timesteps)
